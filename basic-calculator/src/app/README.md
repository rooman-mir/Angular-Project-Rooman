...
Installation Steps
    ...
    git clone https://github.com/[your-username]/Angular-Project-Submission.git
    cd Angular-Project-Submission
...
Explanation:
    ...
    Imports and initializes the AppComponent
...
Key Features:
    ...
    Component Properties: title and displayValue for state management
    Methods: appendValue(), clear(), and calculate() for functionality
    Error Handling: Try-catch block for invalid expressions

3. app.html (Component Template)
...
Key Features:
    ...
    Template Driven Form: Form element wrapping calculator UI
    Two-Way Data Binding: [(ngModel)]="displayValue" for real-time updates
    Control Statements:
        *ngFor for iterating button rows and columns
        [ngClass] for conditional styling
        Ternary operators in (click) for button logic
    Bootstrap Classes: Responsive layout with cards, buttons, and utilities
    Event Binding: (click) events for user interactions

4. app.css (Component Styles)
...
Key Features:
    ...
    Centered Layout: Calculator centered with max-width constraint
    Large Input: Easy-to-read display field
    Bold Buttons: Clear, accessible button text
...
- Used for layout, cards, buttons, and form controls
...
- ✅ Separation of concerns (HTML, CSS, TypeScript)
...
- Support for advanced operations (square root, percentage, etc.)
